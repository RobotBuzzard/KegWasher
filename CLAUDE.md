...
5. **Web config editor** at `http://<machine-ip>/` (port 80, same `kw_remoteCfgApply` path; routes `/`, `/cfg.json`, `/set?KEY=VALUE`). ClearCore TCP-server rules, all bench-bitten: (a) **never call `client.stop()` server-side** — it frees the `TcpData` the server slot table still references (use-after-free → alternating hangs); answer `Connection: close` and let the peer's FIN drive cleanup. (b) **Drain the whole request before responding** — leftover unread bytes keep dead slots alive and get served as the next "request". (c) **Chunk responses ≤1 KB** and pump `Ethernet.maintain()` between chunks (lwIP has a 2.9 KB sndbuf on a 4 KB heap), bailing after 300 ms of no accepted bytes (a dead peer otherwise stalls the loop toward the 8 s WDT).
...
- SD card holds `WASHER.CFG` (timer/threshold/MQTT overrides, plus `netMode`/`netIp`/`netMask`/`netGw` — static IP applied at boot only — and the optional `webUser`/`webPass` editor login). **Filename must be 8.3/UPPERCASE** — the classic Arduino SD library has no long-filename support, so a name like `washer.config` silently fails to open (this was a real bug). Schema is the `KEY=VALUE` format under `config/washer.cfg.example`. If the SD is missing/corrupt, compiled defaults from `KegConfig.cpp` are used and a banner is shown.
