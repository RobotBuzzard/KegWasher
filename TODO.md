...
- ✅ **Heater `ext` mode plumbing (2026-06-12)** — `heaterMode` SD key: `fw` (default, bang-bang STARTING) vs `ext` (ETS50N PNP two-point thermostat keeps the tank always hot; `causticHeaterOut` becomes the safety permit — level/E-stop/sensor/ABORTED/overtemp w/ 2 °C re-arm). Rate/timeout monitors skipped in ext; STARTING exits at the wash floor. **Do not set `ext` until the probe→contactor chain is wired.** Firmware side done; hardware wiring pending.
