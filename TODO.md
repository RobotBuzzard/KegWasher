...
- [x] **Bench mode made runtime (2026-06-10)** — the compile-time `#define BENCH_MODE` is gone. Card present at boot → production gates (verified: dry bench now correctly blocks READY on the LEVEL gate, shown as a 5th readiness row); no card → bench mode (gates bypassed + 5 s stages, loud serial banner + `NONE (BENCH)` boot row). This replaces the old 'switch off BENCH_MODE before production' step — a shipped unit with its config card behaves as production automatically.
...
- ✅ **Full-drain mode (2026-06-12)** — IO2 is a latching illuminated switch: latched ON → DIRTY_DRAIN runs `fullDrainTimer` (default 3 min, panel-settable "FULL DRN", keg-size scaled) for spoiled/undersold kegs that arrive full. Latched at cycle start; live state on retained `kegwasher/mode/fulldrain`. Park/silence moved to a touch **SILENCE** button on COMPLETE (+ `cmd/silence`); diag entry moved to an INFO-page **DIAG** button (the DRAIN+START chord is gone — it would hijack drain-latched starts). Verified on hardware incl. a live ~3 min full-drain stage + closed-loop "Keg at pressure" completion.
- ✅ **Heater `ext` mode plumbing (2026-06-12)** — `heaterMode` SD key: `fw` (default, bang-bang STARTING) vs `ext` (ETS50N PNP two-point thermostat keeps the tank always hot; `causticHeaterOut` becomes the safety permit — level/E-stop/sensor/ABORTED/overtemp w/ 2 °C re-arm). Rate/timeout monitors skipped in ext; STARTING exits at the wash floor. Firmware side done.
- ✅ **Heater `ext` mode hardening for go-live (2026-06-14)** — audited the compile-only ext path before wiring a live contactor: (1) overtemp backstop now raises `ERR_HEATER_OVERTEMP` + aborts (`monitorHeaterExt()`, all states) instead of a silent permit drop — reaching `maxCausticTemp` means the external thermostat failed; (2) ext STARTING is bounded by `MAX_HEATING_TIME` → `ERR_HEATING_TIMEOUT` so a cold/dead thermostat can't hang forever; (3) documented that the permit SSR **must be in series in the contactor-coil circuit** or the level interlock can't cut a dry-firing element. fw regression re-verified. **Pending:** wire the probe→permit→E-stop→contactor chain, set ETS50N SP/RSP, then flip the card to `heaterMode=ext` and wet-verify.
